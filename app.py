...
st.write("Enter the flower measurements below and run a prediction")
...
with st.form("rediction_form"):
    ...
    sepal_width = st.number_input(
        "sepal width (cm)", min_value=0.0, value=5.1, step=0.1
    )
    petel_length = st.number_input(
        "petel length (cm)", min_value=0.0, value=5.1, step=0.1
    )
    petel_width = st.number_input(
        "petel width (cm)", min_value=0.0, value=5.1, step=0.1
    )
